...
type Screen = 'loading' | 'name' | 'quiz' | 'submitting' | 'results' | 'error' | 'full' | 'unavailable';
...
export default function QuizPage({ params }: { params: { id: string } }) {
  ...
  async function submitQuiz() {
    ...
    if (data.score !== undefined) { setResultData(data); setScreen('results'); }
      ...
      if (data.code === 'INACTIVE') { setScreen('unavailable'); }
      else { setScreen('full'); }
    }
  ...
  if (screen === 'unavailable') return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      <div style={{ position: 'fixed', top: 16, right: 16 }}><ThemeToggle /></div>
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div className="card animate-scale" style={{ maxWidth: '460px', textAlign: 'center', padding: '52px 36px' }}>
  ...
  if (screen === 'full') return (
    <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', background: 'var(--bg)' }}>
      <div style={{ position: 'fixed', top: 16, right: 16 }}><ThemeToggle /></div>
      <div className="card animate-scale" style={{ maxWidth: '460px', textAlign: 'center', padding: '48px 32px' }}>
        <p style={{ fontSize: '52px', marginBottom: '16px' }}>🎯</p>
        <h2 style={{ fontWeight: 800, fontSize: '22px', marginBottom: '10px' }}>This Batch is Complete</h2>
        <p className="text-secondary" style={{ fontSize: '15px', lineHeight: 1.6, marginBottom: '24px' }}>
          This quiz has just reached 100 participants. A new batch has automatically opened — please try again in a moment.
        </p>
  ...
  if (screen === 'name') return (
    <main style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', background: 'var(--bg)' }}>
      ...
      <div className="animate-scale" style={{ width: '100%', maxWidth: '480px', position: 'relative', zIndex: 1 }}>
        ...
        <p className="text-center text-muted text-xs" style={{ marginTop: '20px' }}>Quiz Craft by Future Next Technologies</p>
      </div>
    </main>
  );
  ...
  if (screen === 'results' && resultData) {
    ...
    return (
      <main style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="container" style={{ paddingTop: '36px', flex: 1, paddingBottom: '40px' }}>
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="container" style={{ paddingTop: '88px', paddingBottom: '60px', flex: 1 }}>
